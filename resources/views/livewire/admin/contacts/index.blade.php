<div>
    ...
    <div class="mb-5 grid gap-4 lg:grid-cols-[1fr_auto_auto]">
        <div class="relative">
            ...
            >
            <span class="pointer-events-none absolute bottom-3 left-3.5 text-stone-400">
        ...
        <div class="flex items-end">
            <div class="flex w-full items-center justify-between gap-4 rounded-xl bg-pondok-900 px-5 py-3.5 text-white lg:min-w-[12rem]">
                <div>
                    <p class="text-[10px] font-semibold uppercase tracking-[0.16em] text-pondok-100/80">Pesan baru</p>
                    <p class="mt-0.5 font-display text-3xl font-semibold tracking-wide">{{ $unreadCount }}</p>
                </div>
                <svg class="h-10 w-10 text-white/15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 7.5A1.5 1.5 0 0 1 5.5 6h13A1.5 1.5 0 0 1 20 7.5v9a1.5 1.5 0 0 1-1.5 1.5h-13A1.5 1.5 0 0 1 4 16.5v-9Z"/><path stroke-linecap="round" stroke-linejoin="round" d="m5 8 7 5 7-5"/></svg>
            </div>
        </div>
    </div>
                    ...
                    @forelse ($messages as $item)
                        <tr @class(['bg-pondok-50/40' => ! $item->is_read, 'hover:bg-stone-50' => true])>
                            ...
                            <td class="space-x-2 whitespace-nowrap px-4 py-4 text-right">
                                <a href="{{ route('admin.contacts.show', $item) }}" class="font-medium text-pondok-800 underline hover:text-pondok-600">Lihat</a>
                                @unless ($item->is_read)
                                    <button type="button" wire:click="markRead({{ $item->id }})" class="font-medium text-stone-600 underline hover:text-pondok-800">Tandai dibaca</button>
                                @endunless
                                <button
                                    type="button"
                                    wire:click="delete({{ $item->id }})"
                                    wire:confirm="Hapus pesan ini?"
                                    class="font-medium text-rose-600 underline hover:text-rose-800"
                                >Hapus</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center text-stone-500">Belum ada pesan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
